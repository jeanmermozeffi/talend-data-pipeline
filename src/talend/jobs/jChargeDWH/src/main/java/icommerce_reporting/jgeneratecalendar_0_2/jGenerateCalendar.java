		...
		public void synchronizeContext(){
			...
			if(dtDebCalendrier != null){
					...
					if(parts_dtDebCalendrier.length > 1){
						...
						this.setProperty("dtDebCalendrier", pattern_dtDebCalendrier + ";" + FormatterUtils.format_Date(dtDebCalendrier, pattern_dtDebCalendrier));
					}else{
						this.setProperty("dtDebCalendrier", FormatterUtils.format_Date(dtDebCalendrier, pattern_dtDebCalendrier));
					}
			...
			if(dtFinCalendrier != null){
					...
					if(parts_dtFinCalendrier.length > 1){
						...
						this.setProperty("dtFinCalendrier", pattern_dtFinCalendrier + ";" + FormatterUtils.format_Date(dtFinCalendrier, pattern_dtFinCalendrier));
					}else{
						this.setProperty("dtFinCalendrier", FormatterUtils.format_Date(dtFinCalendrier, pattern_dtFinCalendrier));
					}
...
private class TalendException extends Exception {
	...
	public void printStackTrace() {
		...
		try {
			...
			public void tAdvancedHash_joursFeries_error(Exception exception, String errorComponent, final java.util.Map<String, Object> globalMap) throws TalendException {
 ...
 */
  ...
  try{
        if(key_Implicit_Context_Context!=null && "dtDebCalendrier".equals(key_Implicit_Context_Context))
        {
                ...
                if(context_dtDebCalendrier_pos > -1){
                    context_dtDebCalendrier_pattern = context_dtDebCalendrier_value.substring(0, context_dtDebCalendrier_pos);
                }
                context.dtDebCalendrier=(java.util.Date)(new java.text.SimpleDateFormat(context_dtDebCalendrier_pattern).parse(value_Implicit_Context_Context));

        }

        if(key_Implicit_Context_Context!=null && "dtFinCalendrier".equals(key_Implicit_Context_Context))
        {
                ...
                if(context_dtFinCalendrier_pos > -1){
                    context_dtFinCalendrier_pattern = context_dtFinCalendrier_value.substring(0, context_dtFinCalendrier_pos);
                }
                context.dtFinCalendrier=(java.util.Date)(new java.text.SimpleDateFormat(context_dtFinCalendrier_pattern).parse(value_Implicit_Context_Context));

        }

        if(key_Implicit_Context_Context!=null && "nbJoursCalendrier".equals(key_Implicit_Context_Context))
        {
...
context.nbJoursCalendrier = (int)TalendDate.diffDate(context.dtFinCalendrier, context.dtDebCalendrier, "dd")+1;
System.out.println("-----------------------------------------------------------------------------");   
System.out.println("Début de la génération du calendrier : " + 
...
int count_tDBOutput_1=0;
	    java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1.prepareStatement("SELECT COUNT(1) FROM \"" + tableName_tDBOutput_1 + "\" WHERE \"ID_DIM_CALENDRIER\" = ?");
	    resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
	    String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1 + "\" (\"ID_DIM_CALENDRIER\",\"DT_JOUR\",\"NB_ANNEE\",\"NB_SEMESTRE\",\"NB_TRIMESTRE\",\"NB_MOIS\",\"NB_JOUR_ANNEE\",\"NB_JOUR_MOIS\",\"NB_JOUR_SEMAINE\",\"NB_SEMAINE_ANNEE\",\"LB_MOIS\",\"LB_JOUR\",\"BL_DERNIER_JOUR_MOIS\",\"BL_BISSEXTILE\",\"BL_WEEK_END\",\"BL_JOUR_FERIE\",\"LB_JOUR_FERIE\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	    java.sql.PreparedStatement pstmtInsert_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(insert_tDBOutput_1);
	    resourceMap.put("pstmtInsert_tDBOutput_1", pstmtInsert_tDBOutput_1);
	    String update_tDBOutput_1 = "UPDATE \"" + tableName_tDBOutput_1 + "\" SET \"DT_JOUR\" = ?,\"NB_ANNEE\" = ?,\"NB_SEMESTRE\" = ?,\"NB_TRIMESTRE\" = ?,\"NB_MOIS\" = ?,\"NB_JOUR_ANNEE\" = ?,\"NB_JOUR_MOIS\" = ?,\"NB_JOUR_SEMAINE\" = ?,\"NB_SEMAINE_ANNEE\" = ?,\"LB_MOIS\" = ?,\"LB_JOUR\" = ?,\"BL_DERNIER_JOUR_MOIS\" = ?,\"BL_BISSEXTILE\" = ?,\"BL_WEEK_END\" = ?,\"BL_JOUR_FERIE\" = ?,\"LB_JOUR_FERIE\" = ? WHERE \"ID_DIM_CALENDRIER\" = ?";
...
// # Lookup's keys initialization
		...
		org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<joursFeriesStruct> tHash_Lookup_joursFeries = (org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<joursFeriesStruct>) 
				((org.talend.designer.components.lookup.memory.AdvancedMemoryLookup<joursFeriesStruct>) 
					globalMap.get( "tHash_Lookup_joursFeries" ))
					;					
...
detailDate_tmp.DT_JOUR = TalendDate.addDate(context.dtDebCalendrier, nombreDeLigne.NUM_LIGNE, "dd") ;
 					    ...
 					    boolean forceLoopjoursFeries = false;
 							...
 							joursFeriesStruct joursFeriesObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
                        		    		    ...
                        		    		    joursFeriesHashKey.DT_JOUR = detailDate.DT_JOUR  == null ? null : new java.util.Date(detailDate.DT_JOUR .getTime());
				           		...
				           		if(tHash_Lookup_joursFeries != null && tHash_Lookup_joursFeries.getCount(joursFeriesHashKey) > 1) { // G 071
			  							
			  						
									 		
									//System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'joursFeries' and it contains more one result from keys :  joursFeries.DT_JOUR = '" + joursFeriesHashKey.DT_JOUR + "'");
								} // G 071
...
detailCalendrier_tmp.ID_DIM_CALENDRIER = Integer.parseInt(TalendDate.formatDate("yyyyMMdd", detailDate.DT_JOUR)) ;
...
detailCalendrier_tmp.NB_SEMESTRE = TalendDate.getPartOfDate("MONTH", detailDate.DT_JOUR)<6?1:2 ;
detailCalendrier_tmp.NB_TRIMESTRE = TalendDate.getPartOfDate("MONTH", detailDate.DT_JOUR)<3?1:
...
detailCalendrier_tmp.NB_JOUR_ANNEE = TalendDate.getPartOfDate("DAY_OF_YEAR", detailDate.DT_JOUR) ;
detailCalendrier_tmp.NB_JOUR_MOIS = TalendDate.getPartOfDate("DAY_OF_MONTH", detailDate.DT_JOUR) ;
detailCalendrier_tmp.NB_JOUR_SEMAINE = TalendDate.getPartOfDate("DAY_OF_WEEK", detailDate.DT_JOUR)-1==0?7: TalendDate.getPartOfDate("DAY_OF_WEEK", detailDate.DT_JOUR)-1 ;
detailCalendrier_tmp.NB_SEMAINE_ANNEE = TalendDate.getPartOfDate("WEEK_OF_YEAR", detailDate.DT_JOUR) ;
detailCalendrier_tmp.LB_MOIS = TalendDate.formatDate("MMMM", detailDate.DT_JOUR).substring(0, 1).toUpperCase() + 

TalendDate.formatDate("MMMM", detailDate.DT_JOUR).substring(1, TalendDate.formatDate("MMMM", detailDate.DT_JOUR).length()) ;
detailCalendrier_tmp.LB_JOUR = TalendDate.getPartOfDate("DAY_OF_WEEK", detailDate.DT_JOUR)==2?"Lundi":
...
detailCalendrier_tmp.BL_DERNIER_JOUR_MOIS = TalendDate.compareDate(TalendDate.getLastDayOfMonth(detailDate.DT_JOUR), detailDate.DT_JOUR,"yyyy-MM-dd")==0?1:0 ;
detailCalendrier_tmp.BL_BISSEXTILE = ((double)TalendDate.getPartOfDate("YEAR", detailDate.DT_JOUR)/4)==Math.floor(TalendDate.getPartOfDate("YEAR", detailDate.DT_JOUR)/4)?1:0 ;
detailCalendrier_tmp.BL_WEEK_END = TalendDate.getPartOfDate("DAY_OF_WEEK", detailDate.DT_JOUR)==7
...
detailCalendrier_tmp.LB_JOUR_FERIE = joursFeries.DT_JOUR==null?"":joursFeries.LB_JOUR_FERIE ;
...
pstmtUpdate_tDBOutput_1.setTimestamp(1, new java.sql.Timestamp(detailCalendrier.DT_JOUR.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_1.setString(10, detailCalendrier.LB_MOIS);
}

                        if(detailCalendrier.LB_JOUR == null) {
pstmtUpdate_tDBOutput_1.setNull(11, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(11, detailCalendrier.LB_JOUR);
}
...
} else {pstmtUpdate_tDBOutput_1.setString(16, detailCalendrier.LB_JOUR_FERIE);
}

                        pstmtUpdate_tDBOutput_1.setInt(17 + count_tDBOutput_1, detailCalendrier.ID_DIM_CALENDRIER);
...
globalMap.put("tDBOutput_1_ERROR_MESSAGE",e.getMessage());
                    ...
                    whetherReject_tDBOutput_1 = true;
                        ...
                        if(detailCalendrier.DT_JOUR != null) {
pstmtInsert_tDBOutput_1.setTimestamp(2, new java.sql.Timestamp(detailCalendrier.DT_JOUR.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setString(11, detailCalendrier.LB_MOIS);
}

                        if(detailCalendrier.LB_JOUR == null) {
pstmtInsert_tDBOutput_1.setNull(12, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(12, detailCalendrier.LB_JOUR);
}
...
} else {pstmtInsert_tDBOutput_1.setString(17, detailCalendrier.LB_JOUR_FERIE);
}
...
public static class joursFeriesStruct implements routines.system.IPersistableComparableLookupRow<joursFeriesStruct> {
    final static byte[] commonByteArrayLock_ICOMMERCE_REPORTING_jGenerateCalendar = new byte[0];
    static byte[] commonByteArray_ICOMMERCE_REPORTING_jGenerateCalendar = new byte[0];
	protected static final int DEFAULT_HASHCODE = 1;
    protected static final int PRIME = 31;
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_joursFeries";
		...
		int tos_count_tAdvancedHash_joursFeries = 0;
			   		...
			   		// source node:tFileInputExcel_1 - inputs:(after_tRowGenerator_1) outputs:(joursFeries,joursFeries) | target node:tAdvancedHash_joursFeries - inputs:(joursFeries) outputs:()
			   		// linked node: tMap_2 - inputs:(detailDate,joursFeries) outputs:(detailCalendrier)
			   
			   		org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_joursFeries = 
 ...
 */
	...
	final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:FUSIIAQoVFHLEq/mqignOFLxjvuQLD/FbKYxQg==");
		...
		Object source_tFileInputExcel_1 = "/home/jeeff/Bureau/SYNELIA_GROUP/CIC/Talend/formationtalend/Section 10/Mini Projet1 Partie2/joursFeries.xlsx";
				...
				if(0<actual_end_column_tFileInputExcel_1){
					try{
						if(row_tFileInputExcel_1.getCell(columnIndex_tFileInputExcel_1+ start_column_tFileInputExcel_1).getCellType() == org.apache.poi.ss.usermodel.CellType.NUMERIC && org.apache.poi.ss.usermodel.DateUtil.isCellDateFormatted(row_tFileInputExcel_1.getCell(columnIndex_tFileInputExcel_1+ start_column_tFileInputExcel_1))){
							joursFeries.DT_JOUR = row_tFileInputExcel_1.getCell(columnIndex_tFileInputExcel_1+ start_column_tFileInputExcel_1).getDateCellValue();
						}
						else{
                            java.util.Date tempDate_tFileInputExcel_1 = ParserUtils.parseTo_Date(temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], "yyyy-MM-dd");
...
System.out.println("Fin de la génération du calendrier.  : "+ 
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jGenerateCalendar.class.getClassLoader().getResourceAsStream("icommerce_reporting/jgeneratecalendar_0_2/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jGenerateCalendar.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
                ...
                private void processContext_0() {
                        ...
                        if(context.getStringValue("dtDebCalendrier") == null) {
                            ...
                            try{
                                ...
                                if(context_dtDebCalendrier_pos > -1){
                                    context_dtDebCalendrier_pattern = context_dtDebCalendrier_value.substring(0, context_dtDebCalendrier_pos);
                                    context_dtDebCalendrier_value = context_dtDebCalendrier_value.substring(context_dtDebCalendrier_pos + 1);
                                }

                                context.dtDebCalendrier=(java.util.Date)(new java.text.SimpleDateFormat(context_dtDebCalendrier_pattern).parse(context_dtDebCalendrier_value));
                                ...
                                try { 
                                    ...
                                    context.dtDebCalendrier = new java.util.Date(context_dtDebCalendrier_longValue);
                                        ...
                                        System.err.println(String.format("Null value will be used for context parameter %s: %s", "dtDebCalendrier", "Can't parse date string: " + e.getMessage() + " and long: " + cantParseToLongException.getMessage()));
                                    context.dtDebCalendrier=null;
                                }
                            }
                        }
                        ...
                        if(context.getStringValue("dtFinCalendrier") == null) {
                            ...
                            try{
                                ...
                                if(context_dtFinCalendrier_pos > -1){
                                    context_dtFinCalendrier_pattern = context_dtFinCalendrier_value.substring(0, context_dtFinCalendrier_pos);
                                    context_dtFinCalendrier_value = context_dtFinCalendrier_value.substring(context_dtFinCalendrier_pos + 1);
                                }

                                context.dtFinCalendrier=(java.util.Date)(new java.text.SimpleDateFormat(context_dtFinCalendrier_pattern).parse(context_dtFinCalendrier_value));
                                ...
                                try { 
                                    ...
                                    context.dtFinCalendrier = new java.util.Date(context_dtFinCalendrier_longValue);
                                        ...
                                        System.err.println(String.format("Null value will be used for context parameter %s: %s", "dtFinCalendrier", "Can't parse date string: " + e.getMessage() + " and long: " + cantParseToLongException.getMessage()));
                                    context.dtFinCalendrier=null;
                                }
                            }
                        }
                        ...
                        if(context.getStringValue("nbJoursCalendrier") == null) {
                            ...
                            try{
                                context.nbJoursCalendrier=routines.system.ParserUtils.parseTo_Integer (context.getProperty("nbJoursCalendrier"));
                            } catch(NumberFormatException e){
                                System.err.println(String.format("Null value will be used for context parameter %s: %s", "nbJoursCalendrier", e.getMessage()));
                                context.nbJoursCalendrier=null;
                            }
                        }
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("dtDebCalendrier")) {
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jGenerateCalendar");
        }
...
/************************************************************************************************
 *     173220 characters generated by Talend Open Studio for Data Integration 
 *     on the 28 février 2025 à 16:08:05 GMT
 ************************************************************************************************/