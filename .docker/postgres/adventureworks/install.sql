...
DO $$
BEGIN
   IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'adventureworks') THEN
...
-- Activer certaines options de psql
\pset tuples_only on
...
\copy Production.ProductReview FROM '/docker-entrypoint-initdb.d/data/ProductReview.csv' DELIMITER E'\t' CSV;
